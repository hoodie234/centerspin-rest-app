...
public class ArticleComparators {
    ...
    public static final Comparator<JSONObject> AVG_RATING = new Comparator<JSONObject>() {
        @Override
        public int compare(JSONObject article1, JSONObject article2){
            // Return 0 if same
            if(article1.getDouble(Constants.avgRating) == article2.getDouble(Constants.avgRating)) return 0;
